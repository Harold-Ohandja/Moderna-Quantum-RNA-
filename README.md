...
> **WISER Global Quantum+AI Program 2026** | Challenge Moderna  
> **Repository:** [`Moderna-Quantum-RNA`](https://github.com)

---
## 👥 Team

- **Team Member 1:** [Name / Email / GitHub]
- **Team Member 2:** [Name / Email / GitHub]
- **Team Member 3:** [Name / Email / GitHub]
...
This project explores a quantum optimization approach for mRNA folding by mapping base-pairing decisions to a Quadratic Unconstrained Binary Optimization (**QUBO**) model. Inspired directly by research from IBM Quantum and Moderna (*Alevras et al., arXiv:2405.20328*), we implement a **CVaR-VQE (Conditional Value at Risk Variational Quantum Eigensolver)** algorithm using a hardware-efficient ansatz [cite: 1]. We benchmark the quantum predictions against classical MFE structures, evaluate energy gaps (Δ E), and conduct a resource scaling analysis [cite: 1].
...
### 1. Binary Variable Mapping
Instead of mapping individual nucleotides to qubits, variables represent **candidate base pairs** (i, j) that satisfy physical constraints (\(\vert{}j - i\vert{} \ge 4\) and Watson-Crick / Wobble base pairing rules A-U, G-C, G-U).
...
* **Thermodynamic Energy (\(H_{\text{energy}}\)):** Favors stable pairings (G-C < A-U < G-U) and accounts for stacking bonuses [cite: 1].
* **Overlap Constraint (\(H_{\text{overlap}}\)):** Penalizes configurations where a single base forms multiple bonds (\(P_{\text{overlap}} \cdot x_{k_1} x_{k_2}\)) [cite: 1].
* **Pseudoknot Exclusion (\(H_{\text{pseudoknots}}\)):** Penalizes crossing pairs (i₁ < i₂ < j₁ < j₂) to maintain nested secondary structures [cite: 1].

### 3. CVaR-VQE Algorithm
Standard VQE minimizes expected energy over all samples, which can struggle in rugged combinatorial landscapes [cite: 1]. We employ **CVaR-VQE**, optimizing only the expectation value of the best α-quantile (α = 0.1) of sampled bitstrings, drastically accelerating convergence toward the ground state [cite: 1].
