...
\begin{document}
	...
	\begin{titlepage}
		...
		{\large Pushkar Kumar --- Quantum Solver Implementation \& Benchmarking}\\[0.15cm]
		{\large Harold Ohandja --- QUBO Formulation, Project Direction \& Resource Analysis}
...
+-------------------------------------------------------------------------+
	...
	\begin{figure}[htbp]
		...
		\caption{Empirically measured qubit count (candidate-pair count) versus RNA sequence length on random sequences. The dashed line marks the roughly 16-qubit threshold we adopted for fast local simulation in this project (not a hard memory limit); the curve crosses it between 12 and 14 nucleotides. The non-monotonic behavior around 20 nt reflects the random sequence composition at each length.}
		\label{fig:scaling}
	\end{figure}
	
	\subsection{Interpretation: Theory vs. Empirical}
	
	The two views tell a consistent story. The theoretical bound establishes that $N$ can in principle grow as $\mathcal{O}(L^2)$, but the empirical measurement shows that on real sequences the count rises more gently and irregularly, dominated by how many canonical pairs happen to be compatible. We restricted our quantum experiments to roughly $\le 16$ qubits to keep local statevector simulation fast on modest hardware; this is a practical project constraint, not a hard memory limit, since exact statevector simulation remains feasible to approximately 30 qubits (16~GB). The deeper scaling constraint is the one-qubit-per-candidate-pair encoding itself, which reaches 87 qubits at 23~nt and points to a stem-level encoding as the key improvement for longer sequences. Because the demonstrated instances are small, the sampler explores most of the state space, so the reported results should be read as feasibility demonstrations rather than as evidence of quantum optimization advantage.
	
	\subsection{Circuit Depth and Gate Scaling}
	
	Using the hardware-efficient \texttt{n\_local} ansatz, circuit depth and gate counts scale linearly with $N$. The depth reported by \texttt{quantum/resource\_analysis.py} is measured directly by transpiling the ansatz to the $\{R_y, \text{CZ}\}$ basis (rather than assumed), and grows from 5 layers at $N=2$ to 82 layers at $N=78$, consistent with the linear expectation below:
	\begin{itemize}[leftmargin=1.2cm]
	...
	\begin{itemize}[leftmargin=1.2cm]
		\item \textbf{Pushkar Kumar:} Quantum solver implementation (CVaR-VQE, QAOA), classical baseline solvers (brute-force and greedy heuristic), benchmarking notebooks and the unified comparison, the noise-robustness study, the interactive web explainer, and report preparation.
		\item \textbf{Harold Ohandja:} QUBO/Ising formulation and core modules, project direction and roadmap, quantum resource and scaling analysis, and benchmark tooling.
	\end{itemize}
